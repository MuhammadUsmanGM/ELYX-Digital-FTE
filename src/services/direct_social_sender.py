...
try:
    from src.utils.rate_limiter import wait_for_rate_limit, check_rate_limit
except ImportError:
    def wait_for_rate_limit(s): return 0
    def check_rate_limit(s): return True
...
def _find_and_click(page, selectors: list, timeout: int = 5000, description: str = "element"):
...
def _find_element(page, selectors: list, timeout: int = 5000, description: str = "element"):
...
def _not_logged_in(platform: str):
    return {"success": False, "error": f"Not logged in to {platform}. Run: python setup_sessions.py {platform.lower()}"}
...
def send_gmail_via_browser(to: str, subject: str, body: str, session_path: str = None):
    ...
    try:
        ...
        # Step 2: Open Compose
        compose_opened = _find_and_click(page, [
            'div.T-I.T-I-KE.L3[role="button"]',
            '[gh="cm"]',
            'div[role="button"]:has-text("Compose")',
        ], timeout=8000, description="Compose button")
        ...
        to_field = _find_element(page, [
            'textarea[aria-label="To recipients"]',
            'textarea[name="to"]',
            'input[aria-label="To recipients"]',
            'input[name="to"]',
            'input[peoplekit-id="BbVjBd"]',
        ], timeout=8000, description="To field")

        if not to_field:
            return {"success": False, "error": "Could not find To field in compose window"}
        ...
        # Step 5: Fill Subject
        subj_field = _find_element(page, [
            'input[aria-label="Subject"]',
            'input[name="subjectbox"]',
        ], timeout=3000, description="Subject field")
        if subj_field:
            subj_field.click()
        ...
        # Step 6: Fill Body
        body_field = _find_element(page, [
            'div[aria-label="Message Body"][contenteditable="true"]',
            'div[role="textbox"][aria-label="Message Body"]',
            'div.Am.Al.editable[contenteditable="true"]',
            'div[contenteditable="true"][g_editable="true"]',
        ], timeout=3000, description="Body field")
        ...
        return {"success": True, "message": f"Email sent to {to} via Gmail", "platform": "gmail"}
...
def send_gmail_via_api(to: str, subject: str, body: str):
    ...
    try:
        ...
        if not Path(creds_path).exists():
            return {"success": False, "error": f"Gmail credentials not found at {creds_path}"}
        ...
        return {"success": True, "message": f"Email sent to {to} via Gmail API", "platform": "gmail",
...
def send_linkedin_post(message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=8000, description="Start a post"):
        ...
        ], timeout=8000, description="Post editor")
        ...
        ], timeout=8000, description="Post button"):
            page.keyboard.press("Control+Enter")

        time.sleep(3)
        return {"success": True, "message": "LinkedIn post published", "platform": "linkedin"}
...
def send_linkedin_dm(recipient_name: str, message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=5000, description="Contact thread")
        time.sleep(1)

        # Type message
        msg_box = _find_element(page, [
            'div.msg-form__contenteditable[contenteditable="true"]',
            'div[role="textbox"][aria-label*="Write a message"]',
            'div.msg-form__msg-content-container div[contenteditable="true"]',
        ], timeout=8000, description="Message box")
        ...
        ], timeout=5000, description="Send button"):
            page.keyboard.press("Enter")

        time.sleep(2)
        return {"success": True, "message": f"LinkedIn DM sent to {recipient_name}", "platform": "linkedin"}
...
def send_facebook_post(message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=8000, description="Create post"):
        ...
        time.sleep(3)
        return {"success": True, "message": "Facebook post published", "platform": "facebook"}
...
def send_facebook_dm(recipient: str, message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=8000, description="Messenger search")
        ...
        if search:
            ...
            ], timeout=5000, description="Contact result")
            time.sleep(2)
        ...
        page.keyboard.press("Enter")
        time.sleep(2)
        return {"success": True, "message": f"Facebook DM sent to {recipient}", "platform": "facebook"}
...
def send_tweet(message: str, session_path: str = None):
    ...
    try:
        ...
        # Fill tweet
        tweet_box = _find_element(page, [
            '[data-testid="tweetTextarea_0"]',
            'div[role="textbox"][data-testid="tweetTextarea_0"]',
            'div[contenteditable="true"][role="textbox"]',
        ], timeout=8000, description="Tweet box")
        ...
        ], timeout=8000, description="Tweet button"):
            page.keyboard.press("Control+Enter")
...
def send_twitter_dm(username: str, message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=8000, description="New DM button")
        time.sleep(2)
        ...
        ], timeout=8000, description="People search")
        ...
        if search:
            ...
            ], timeout=5000, description="User result")
            time.sleep(1)
            ...
            ], timeout=5000, description="Next button")
            time.sleep(1)
        ...
        ], timeout=8000, description="DM input")
        ...
        time.sleep(2)
        return {"success": True, "message": f"Twitter DM sent to {username}", "platform": "twitter"}
...
def send_instagram_post(message: str, image_path: str = None, session_path: str = None):
    ...
    if not image_path or not Path(image_path).exists():
        return {"success": False, "error": f"Instagram feed post requires an image. Got: {image_path}"}
    ...
    try:
        ...
        if not _find_and_click(page, [
            ...
            '[aria-label="New post"]',
            'a[href="/create/style/"]',
        ], timeout=8000, description="New post"):
            ...
            for item in nav_items:
                ...
                if 'new' in label.lower() or 'create' in label.lower():
                    ...
                    break
            if not clicked:
                return {"success": False, "error": "Could not find Create/New Post button"}
        ...
        _find_and_click(page, ['button:has-text("Next")', 'div[role="button"]:has-text("Next")'], timeout=5000, description="Next (crop)")
        time.sleep(2)

        # Click Next (filter step)
        _find_and_click(page, ['button:has-text("Next")', 'div[role="button"]:has-text("Next")'], timeout=5000, description="Next (filter)")
        time.sleep(2)

        # Add caption
        caption_input = _find_element(page, [
            'textarea[aria-label*="caption"]',
            'textarea[aria-label*="Write"]',
        ], timeout=5000, description="Caption input")
        ...
        ], timeout=8000, description="Share button"):
            return {"success": False, "error": "Could not find Share button"}

        time.sleep(5)
        return {"success": True, "message": "Instagram post published", "platform": "instagram"}
...
def send_instagram_dm(recipient: str, message: str, session_path: str = None):
    ...
    try:
        ...
        ], timeout=8000, description="New message")
        time.sleep(2)
        ...
        search = _find_element(page, [
            ...
            'input[name="queryBox"]',
        ], timeout=8000, description="Search box")
        ...
        if search:
            ...
            ], timeout=5000, description="Chat/Next button")
            time.sleep(1)
        ...
        ], timeout=8000, description="Message input")
        ...
        time.sleep(2)
        return {"success": True, "message": f"Instagram DM sent to {recipient}", "platform": "instagram"}
...
def send_whatsapp_message(phone: str, message: str, session_path: str = None):
    ...
    try:
        ...
        try:
            page.wait_for_selector('[data-testid="chat-list"], #pane-side, [aria-label="Chat list"]', timeout=20000)
        except Exception:
            return _not_logged_in("WhatsApp")

        # Search for contact
        search_box = _find_element(page, [
        ...
        if not search_box:
            ...
            _find_and_click(page, ['[data-testid="search"]', '[data-icon="search"]'], timeout=5000, description="Search icon")
            time.sleep(1)
            search_box = _find_element(page, ['div[contenteditable="true"][data-tab="3"]'], timeout=5000, description="Search box (retry)")
        ...
        return {"success": True, "message": f"WhatsApp sent to {phone}", "platform": "whatsapp"}
...
def send_message(platform: str, recipient: str, content: str, **kwargs) -> dict:
    ...
    dispatch = {
        "gmail":         lambda: send_gmail_via_browser(to=recipient, subject=kwargs.get("subject", ""), body=content),
        "email":         lambda: send_gmail_via_browser(to=recipient, subject=kwargs.get("subject", ""), body=content),
        ...
        "instagram":     lambda: send_instagram_post(content, image_path=kwargs.get("image_path")),
        "instagram-post":lambda: send_instagram_post(content, image_path=kwargs.get("image_path")),
...
def verify_session(platform: str, session_path: str = None):
    ...
    platforms = {
        "linkedin":  {"url": "https://www.linkedin.com/feed/",  "session": "sessions/linkedin_session"},
        "facebook":  {"url": "https://www.facebook.com/",       "session": "sessions/facebook_session"},
        "twitter":   {"url": "https://x.com/home",              "session": "sessions/twitter_session"},
        "instagram": {"url": "https://www.instagram.com/",      "session": "sessions/instagram_session"},
        "gmail":     {"url": "https://mail.google.com/mail/",   "session": "sessions/gmail_session"},
    }
    ...
    try:
        ...
        if "login" in page.url.lower() or "signin" in page.url.lower() or "accounts" in page.url.lower():
...
def main():
    ...
    if command.startswith("verify-"):
        ...
        print(f"{'[OK]' if result['success'] else '[FAIL]'} {plat.title()}: {result.get('message') or result.get('error')}")
    ...
    print(f"{'[OK]' if result['success'] else '[FAIL]'} {result.get('message') or result.get('error')}")
    sys.exit(0 if result["success"] else 1)
