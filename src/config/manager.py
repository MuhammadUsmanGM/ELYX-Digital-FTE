...
class ConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        # merge any secrets from vault or env into config tree
        self._inject_secrets(cfg)
        return cfg

    def _inject_secrets(self, cfg: Dict[str, Any]) -> None:
        """
        Overlay secrets from the environment or a vault directory into
        the loaded configuration. This allows sensitive values to be stored
        securely outside of version control.
        """
        # simple example: look for env vars matching CONFIG_<path> and inject
        for env_key, env_val in os.environ.items():
            if env_key.startswith("CONFIG_"):
                # CONFIG_section__subkey=value -> cfg['section']['subkey']
                path = env_key[len("CONFIG_") :].lower().split("__")
                d = cfg
                for p in path[:-1]:
                    d = d.setdefault(p, {})
                d[path[-1]] = env_val
        ...
        vault_dir = os.getenv("VAULT_SECRETS_PATH") or os.path.join(
            cfg.get("vault_path", ""), "Secrets"
        )
                ...
                if os.path.isfile(full):
                    try:
                        with open(full, "r", encoding="utf-8") as f:
                            value = f.read().strip()
                        # place at top level using filename as key
                        cfg[fname] = value
                    except Exception:
                        pass
    ...
    def get_secret(self, key: str, default: Optional[Any] = None) -> Any:
        """
        Convenience helper to retrieve a configuration value which may be
        stored in a secret or injected via environment. If the key does not
        exist, returns `default`.

        The key may be a dotted path (e.g. "gmail.credentials_path").
        """
        parts = key.split(".")
        d = self.config
        for p in parts:
    ...
    def get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "check_interval": {
                ...
                "filesystem": int(
                    os.getenv("FILESYSTEM_CHECK_INTERVAL", "10")
                ),  # seconds
                "calendar": int(os.getenv("CALENDAR_CHECK_INTERVAL", "300")),  # seconds
                "orchestrator": int(
                    os.getenv("ORCHESTRATOR_CHECK_INTERVAL", "60")
                ),  # seconds
            },
            "watchdog": {
                "check_interval": int(os.getenv("WATCHDOG_CHECK_INTERVAL", "60")),
                "auto_restart": os.getenv("WATCHDOG_AUTO_RESTART", "true").lower()
                == "true",
            },
            "gmail": {
            ...
            "linkedin": {
                ...
                "check_interval": int(
                    os.getenv("LINKEDIN_CHECK_INTERVAL", "300")
                ),  # seconds
            },
            "calendar": {
                "sync_enabled": os.getenv("CALENDAR_SYNC_ENABLED", "false").lower()
                == "true",
                "providers": ["google", "outlook"],  # Supported providers
                "default_provider": os.getenv("CALENDAR_DEFAULT_PROVIDER", "google"),
                "sync_frequency_minutes": int(
                    os.getenv("CALENDAR_SYNC_FREQUENCY", "15")
                ),
                "create_tasks_for_events": os.getenv(
                    "CALENDAR_CREATE_TASKS_FOR_EVENTS", "true"
                ).lower()
                == "true",
            },
            "silver_tier_features": {
                ...
                == "true",
                "enable_learning": os.getenv("ENABLE_LEARNING", "true").lower()
                == "true",
                "enable_advanced_monitoring": os.getenv(
                    "ENABLE_ADVANCED_MONITORING", "true"
                ).lower()
                == "true",
                "enable_predictive_features": os.getenv(
            ...
            "gold_tier_features": {
                "enable_advanced_ai": os.getenv("ENABLE_ADVANCED_AI", "true").lower()
                == "true",
                "enable_strategic_insights": os.getenv(
                    "ENABLE_STRATEGIC_INSIGHTS", "true"
                ).lower()
                == "true",
                "enable_collaboration_engine": os.getenv(
            ...
            "integrations": {
                ...
                "project_management_enabled": os.getenv(
                    "PROJECT_MANAGEMENT_ENABLED", "false"
                ).lower()
                == "true",
                "gmail_enabled": os.getenv("GMAIL_ENABLED", "true").lower() == "true",
                "whatsapp_enabled": os.getenv("WHATSAPP_ENABLED", "true").lower()
                == "true",
                "linkedin_enabled": os.getenv("LINKEDIN_ENABLED", "true").lower()
                == "true",
            },
            ...
            "global": {
                ...
                "scale": {
                    "enabled": os.getenv("GLOBAL_SCALE_ENABLED", "true").lower()
                    == "true",
                    "multi_region_support": os.getenv(
            ...
            "quantum": {
                "encryption": {
                    "enabled": os.getenv("QUANTUM_ENCRYPTION_ENABLED", "true").lower()
                    == "true"
                },
                "key_rotation_interval": int(
                    os.getenv("QUANTUM_KEY_ROTATION_INTERVAL", "24")
                ),  # hours
                "secure_communication": os.getenv(
            ...
            "iot": {
                "device_manager": {
                    "enabled": os.getenv("IOT_DEVICE_MANAGER_ENABLED", "true").lower()
                    == "true"
                },
                "device_api_key": os.getenv("IOT_DEVICE_API_KEY", ""),
                "connection_timeout": int(
                    os.getenv("IOT_DEVICE_CONNECTION_TIMEOUT", "30")
                ),
            },
            # AR/VR interface configuration
            "arvr": {
                "interface": {
                    "enabled": os.getenv("ARVR_INTERFACE_ENABLED", "true").lower()
                    == "true"
                },
            ...
            "global_scaling": {
                "load_balancer": os.getenv(
                    "GLOBAL_LOAD_BALANCER", "http://global-lb.example.com"
                ),
                "regional_endpoints": json.loads(
                    os.getenv(
                        "REGIONAL_ENDPOINTS",
                        '["http://us-east.example.com", "http://eu-west.example.com"]',
                    )
                ),
                "auto_scaling_enabled": os.getenv(
            ...
            "ai": {
                ...
                "quantum_processor_provider": os.getenv(
                    "QUANTUM_PROCESSOR_PROVIDER", "qiskit"
                ),
                "federated_learning_enabled": os.getenv(
            ...
            "platinum_tier_features": {
                ...
                == "true",
                "enable_iot_connectivity": os.getenv(
                    "ENABLE_IOT_CONNECTIVITY", "true"
                ).lower()
                == "true",
                "enable_arvr_interfaces": os.getenv(
                    "ENABLE_ARVR_INTERFACES", "true"
                ).lower()
                == "true",
                ...
                == "true",
                "enable_autonomous_operations": os.getenv(
